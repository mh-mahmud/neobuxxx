...
class Mbanking_controller extends CI_Controller {
	...
	public function mobile_banking_send_money($data=null) {

		$data['confirm_page'] = false;
		$url_prefix = $this->webspice->settings()->site_url_prefix;
		$this->webspice->user_verify($url_prefix.'login', $url_prefix.'mobile_banking_send_money');
		$this->webspice->permission_verify('mobile_banking_send_money');
		...
		*/
		...
		$this->form_validation->set_rules('flexi_type', 'type', 'required|trim|xss_clean');
		
		if( !$this->form_validation->run() ){
			$this->load->view('admin_new/mbanking/mobile_banking_send_money', $data);
			return FALSE;
		}
		...
		if($this->webspice->permission_verify('mobile_view_send_money',TRUE)){
			$this->webspice->force_redirect($url_prefix . 'mobile_view_send_money/confirm/'.$my_str);
			return FALSE;
		}
		$this->webspice->force_redirect($url_prefix.'mobile_view_send_money/confirm/'.$my_str);
	}

	public function mobile_view_send_money() {

		$user_sess_id = $this->webspice->enc($_SESSION['user']['USER_ID'], 'decrypt');
		$url_prefix = $this->webspice->settings()->site_url_prefix;
		$this->webspice->user_verify($url_prefix.'login', $url_prefix.'mobile_view_send_money');
		$this->webspice->permission_verify('mobile_view_send_money');
		$this->load->database();
		$orderby = 'ORDER BY client_address_book.SRV_ID DESC';
		$groupby = null;
		$where = '';
		...
		$initialSQL = "
		SELECT  * FROM client_address_book ";


		# filtering records
		if( $this->input->post('filter') ){
			$result = $this->webspice->filter_generator(
				$TableName = 'client_address_book',
				$InputField = array(),
			...
			case 'confirm':
				...
				*/
				...
				if(!$this->webspice->admin_verify() && $data['amount'] > $balance) {
					...
					$this->webspice->force_redirect($url_prefix . 'mobile_banking_send_money');
					return false;
				}
				...
				if( !$this->form_validation->run() ){
					$this->load->view('admin_new/mbanking/confirm_send_money', $data);
					return FALSE;
				}
				...
				$trans_type = "mobile_banking";

				$sql = "
				INSERT INTO client_address_book
				...
				if($this->webspice->permission_verify('mobile_view_send_money',TRUE)){
					$this->webspice->force_redirect($url_prefix . 'mobile_view_send_money');
					return FALSE;
				}
				$this->webspice->force_redirect($url_prefix.'mobile_banking_send_money');
		...
		$this->load->view('admin_new/mbanking/mobile_view_send_money', $data);

	}

	public function mobile_bulk_upload() {

		$url_prefix = $this->webspice->settings()->site_url_prefix;
		$this->webspice->user_verify($url_prefix.'login', $url_prefix.'mobile_bulk_upload');
		$this->webspice->permission_verify('mobile_bulk_upload');
		$data = array();
		$errors = array();

		if( !$_FILES || !$_FILES['upload_data_file']['tmp_name'] ){
			$this->load->view('admin_new/mbanking/mobile_bulk_upload', $data);
			return FALSE;
		}
		...
		if( $_FILES['upload_data_file'] ) {
			...
			$this->webspice->force_redirect($url_prefix.'mobile_bulk_upload');
